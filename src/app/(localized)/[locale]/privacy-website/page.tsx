...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildSeoMetadata(locale, 'privacy-website');
}

export default async function WebsitePrivacyPolicyPage({ params }: Props) {
  const { locale } = await params;
  if (!locales.includes(locale) || locale === defaultLocale) notFound();
  ...
  return (
    <main>
      <Header />
      <LegalDocument locale={locale} document="privacy-website" />
      <Cta />
      <SiteFooter />
    </main>
  );
